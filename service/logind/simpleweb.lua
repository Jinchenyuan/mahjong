...
local route = require "logindweb.urls"
local table = table
local string = string
...
if mode == "agent" then
	...
	local url = skynet.getenv("http")
	local address, port = string.match(url, "([%d.]+)%:(%d+)")
	skynet.start(
		function()
			local agent = {}
			local protocol = "http"
			...
			skynet.error(string.format("Listen web port %d protocol:%s", port, protocol))
			socket.start(
				id,
				function(id, addr)
