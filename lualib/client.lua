...
local function request(fd, name, args, response)
	...
	if f then
		local ok, result = xpcall(f, traceback, fd, args)
		if ok then
			if result then
...
local function response(fd, session, args)
	local name = response_session_name[session]
	local f = RESPONSE[name]
	if f then
		local ok, result = pcall(f, fd, args)
		if not ok then
			log.error(result)
		end
	else
