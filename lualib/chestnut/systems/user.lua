...
function _M:modify_name(args)
	self.mod_user.nickname = args.nickname
	self.mod_user.y = true
	log.info("uid(%d) REQUEST = [modify_name], nickname = [%s]", self.uid, self.mod_user.nickname)
	local ret = {
		errorcode = 0,
		uid = self.uid,
		nickname = self.mod_user.nickname
	}
	client.push(self, "new_name", ret)
end
