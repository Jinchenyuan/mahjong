...
function _M:write_events(data)
	local sql = string_format([==[CALL
		sp_events_insert_or_update('%s', '%s');]==], data.key, data.callback)
	local res = self.db:query(sql)
	if res.errno then
		log.error(sql)
		log.error("%s", self.dump(res))
...
function _M:write_account(db_account)
	...
	skynet.error("write account over")
end

function _M:write_nameid(data)
	local sql = string_format([==[CALL
		sp_nameid_insert_or_update('%s', %d);]==], data.nameid, data.uid)
	log.debug("sql = %s", sql)
	local res = self.db:query(sql)
	if res.errno then
		log.error("%s", self.dump(res))
